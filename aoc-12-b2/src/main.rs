...
type MatrixEntry = i64;

type MatrixGenerationData = (
    (isize, BTreeSet<isize>),
    (isize, BTreeSet<isize>),
    BTreeSet<isize>,
);
type MatrixCache = RefCell<HashMap<MatrixGenerationData, DMatrix<MatrixEntry>>>;
...
fn solve_problem(input: &str) -> MatrixEntry {
    ...
    let matrix_cache = Rc::new(RefCell::new(HashMap::new()));
    let mut total = 0;
    for mut problem in problems.into_iter() {
        expand_problem(&mut problem);
        total += total_solutions(&matrix_cache, &problem);
    }
    return total;
}
...
fn causality_matrices(
    ...
    for (first, second) in block_positions.iter().zip(block_positions.iter().skip(1)) {
        let gen_data = generation_data(problem, first, second);
        let mut cache = matrix_cache.borrow_mut();
        let matrix = cache
            .entry(gen_data)
            .or_insert_with_key(|gen| allowance_matrix(&gen));
        matrices.push(matrix.clone());
    }
    return matrices;
}

// To generate the allowance matrix for two adjacent blocks, we need:
// > the size of each block and all of their prospective locations
// > the locations of all broken springs that could possibly lie between them
fn generation_data(
    problem: &SpringProblem,
    first: &(usize, LocationSet),
    second: &(usize, LocationSet),
) -> MatrixGenerationData {
    let (first_size, mut first_locs) = first.clone();
    let (second_size, mut second_locs) = second.clone();
    let min = *first_locs.first().unwrap();
    let max = *second_locs.last().unwrap();

    let broken_locs: BTreeSet<isize> = problem.states[min..max]
        .iter()
        .copied()
        .enumerate()
        .filter(|(_, x)| *x == State::Broken)
        .map(|(i, _)| i as isize)
        .collect();

    let first_size = first_size as isize;
    let second_size = second_size as isize;
    let first_locs = first_locs
        .iter()
        .map(|x| (*x as isize) - (min as isize))
        .collect();
    let second_locs = second_locs
        .iter()
        .map(|x| (*x as isize) - (min as isize))
        .collect();
    return (
        (first_size, first_locs),
        (second_size, second_locs),
        broken_locs,
    );
}
...
fn allowance_matrix(gen_data: &MatrixGenerationData) -> DMatrix<MatrixEntry> {
    let ((first_size, first_locs), (_second_size, second_locs), broken_locs) = gen_data;
    ...
    for first_pos in first_locs.iter() {
        ...
        for second_pos in second_locs.iter() {
            ...
            if let Some(broken_pos) = first_broken(&broken_locs, lb) {
...
fn first_broken(broken_locs: &BTreeSet<isize>, start: isize) -> Option<isize> {
    return broken_locs.range(start..).copied().next();
}
...
fn total_solutions(matrix_cache: &MatrixCache, spring_problem: &SpringProblem) -> MatrixEntry {
    let mut block_positions = generate_location_sets(spring_problem);
    neighbor_prune(&mut block_positions);
    let matrices = causality_matrices(&matrix_cache, spring_problem, &block_positions);
